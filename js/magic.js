...
// Verb-Object-Subject: fetchCSVhk = (do)Fetch-(on)CSV-(for)HouseKeeping Data.
//                    : refreshPSUstatus = (do)Refresh - (on)PSU - (for)Status Info.
...
function fetchCSVhk() {
  ...
  $(".alertMessage").append(
    "<div class='alert alert-success alert-dismissable'><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>Success! your CSV should be downloading right now.</div>"
  );
}
...
function fetchCSVph() {
  ...
  window.location.assign(url);
  $(".alertMessage").append(
    "<div class='alert alert-success alert-dismissable'><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>Success! your CSV should be downloading right now.<br>If not, please check the console for the error message.</div>"
  );
}
...
function refreshPHChart() {
  ...
  $("#LoadChart").prop(
    "title",
    "Reloads the chart with new data (if available)."
  );
}
...
function refreshVoltageChart() {
  ...
  function updateChart() {
    $.getJSON("/api/psu", function(data) {
      ...
      if (closeFunction) {
        //Check if someone has decided to stop rendering? If yes, exit this function.
        return true;
      }

      setTimeout(function() {
        updateChart();
      }, 1000); //Refresh this function after 1000ms, in case the user hasn't decided to stop this function.
    });
  }
